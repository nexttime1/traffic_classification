...
import os
import random
# from utlis import ID_TO_APP, ID_TO_TRAFFIC  # 只保留用到的导入  
from utlis import ID_TO_TorAPP, ID_TO_TRAFFIC  #这个是tor
...
if __name__ == '__main__':
    # 重点：修改为你实际的预处理输出路径（dpProcess_vpn.py生成的datanet_service.txt）
    # inpath_service = "./service/datanet_service.txt"  # 预处理后service文件夹下的TXT
    # 重点：修改为你想保存训练/测试集的路径（建议与模型训练路径对应）
    # outpath_service = "../datasets/MATEC/service/data"  # 确保上级目录存在
    
    inpath_app = "./app/datanet_app.txt"
    # 输出：TOR数据集的训练/验证/测试集存放路径（自己建个tor_data文件夹）
    outpath_app = "../datasets/MATEC/app/data"
